...
우선 생각해낸 방법은 window.addEventListener를 활용하여 이벤트를 감지하는 것이었다. 페이지 이동 외에도 새로고침, 뒤로 가기를 함께 확인해줘야 하는데 세 이벤트를 한번에 묶어서 하나의 커스텀 훅으로 만들었다.
...
popstate 이벤트는 브라우저의 기록을 탐지하는 이벤트로 페이지의 뒤로 가기, 앞으로 가기 등을 탐지한다. 브라우저의 History API로 페이지의 이동을 조절하고 상태를 관리할 수 있다.
...
뒤로 가기를 클릭하면 `handlePopState`가 실행되는데 뒤로 가기 클릭과 동시에 history는 하나 뒤로 이동한다. **따라서 `useEffect`에서 추가된 현재 경로는 state 스택에서 제거되고 스택 최상단에는 이전 경로가 포함되어있다.** 이 상태에서 뒤로 가기를 한 번 더 클릭하면 스택 최상단인 이전 경로로 이동하기 때문에 만약 뒤로 갈 수 없는 상황(변경 사항이 있는 상태 = !isSaved)에서는 현재 경로를 한 번 더 추가하여 history 스택에 현재 경로를 다시 추가해야 한다.

```
const isClickedFirst = useRef(false);

const handlePopState = useCallback(() => {
  // 1. 뒤로 가기를 클릭한 순간 useEffect에서 실행된 ✨ 가 바로 제거된다.
  if (!isSaved) {
...
페이지 이동은 처음엔 click 이벤트를 사용했으나 클릭할 때마다 리스너가 실행되기 때문에 페이지 이동이라는 목적과 맞지 않는다 판단하여 새로운 방법을 찾아냈다. Vercel에서 알려준 usePathname은 페이지 렌더링 이후 실행되기 때문에 소용이 없었고 next.js github discussion에서 [https://github.com/vercel/next.js/discussions/41934](https://github.com/vercel/next.js/discussions/41934)에 달린 댓글을 참고하여 만들었더니 문제 없이 잘 되었다!

여기서, newPush 라는 새로운 함수를 만들어 router.push를 새로운 함수로 대체하여 페이지 이동을 다룬다. 기존의 router.push 대신 새로운 함수가 실행되기에 함수 로직 내에 특정 조건에 해당할 경우 모달을 띄우도록 할 수 있다. 만약 업데이트된 상태가 아니라면 기존 router.push로 페이지 이동을 하도록 한다.
