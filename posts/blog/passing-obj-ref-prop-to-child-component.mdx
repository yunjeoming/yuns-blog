...
곰곰이 생각해보니 원래 자식 컴포넌트에 하나의 ref를 전달하면, 그 자식 컴포넌트는 forwardRef로 원래의 ref 타입 즉 React.MutableRefObject\<T>을 받기 때문에 ref는 무조건 ref 타입으로 보내야겠다 싶었다.
...
위 ❌의 예시에서 ref에 바로 current.ref1을 넣어주는 것을 볼 수 있는데 이렇게 바로 넣어줄 경우, 아래와 같은 타입 에러가 발생한다.
